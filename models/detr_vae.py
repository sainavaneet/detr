    ...
    def forward(self, qpos, image, env_state, actions=None, is_pad=None):
        ...
        if self.backbones is not None:
            ...
            for cam_id, cam_name in enumerate(self.camera_names):
                # print("Image shape before passing to backbone:=======", image.shape)
                # print(f"Camera ID: {cam_id}, Image Shape: {image[:, cam_id].shape}")
                # dummy_img = torch.rand(1, 3, 480, 640)  # Creating a dummy 3-channel image
                # features, pos = self.backbones[0](image[:, cam_id])
            
                features, pos = self.backbones[cam_id](image)
                    
                
                # features, pos = self.backbones(image)
...
class CNNMLP(nn.Module):
    ...
    def forward(self, qpos, image, env_state, actions=None):
        ...
        for cam_id, cam_name in enumerate(self.camera_names):
            ...
            print(f"Features shape for {cam_name}: {features.shape}")
            features = features[0] # take the last layer feature
            pos = pos[0] # not used
